...
(cd "$S11_ROOT/backend" && go test -tags=integration ./internal/httpapi ./internal/identity \
  -run 'Test(CompleteStudentAuthenticationJourney|BatchA_CourseAccessInvitationHTTPAPI_RealPostgreSQL|BatchB_CourseAccessGrant_RealPostgreSQL|BatchB_GrantConcurrency_RealPostgreSQL|DenialsAreByteIdentical|EveryProtectedLearningRouteRevalidates|EveryDenialLeavesEntitlementEnrollmentAndProgressUnchanged|VerificationResendSupersedesAndConsumptionIsSingleUse|ResetSecretRefusesExpiredReplayedAndWrongPurpose)$' \
  -count=1)

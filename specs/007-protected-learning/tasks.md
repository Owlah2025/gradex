...
- [ ] T077 Local gate: `gofmt -l .`, `go build ./...`, `go vet ./...`, `go vet -tags=integration ./...`, `go test -race ./...` in `backend/`; `npm run lint`, `npm run build`, `npx playwright test` in `frontend/`; and `git status --porcelain` **empty**

...
export function TaxonomyControls({
  ...
  courseID: string;
  /** The submitted revision, when the Course was opened from the review queue. */
  ...
  return (
    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      {error ? <p className="text-xs text-rose-600">{error}</p> : <><TaxonomyTermManagement terms={terms} refresh={refresh} /><TaxonomyOverrideForm courseID={courseID} terms={terms} defaultRevisionID={defaultRevisionID} /></>}
    </section>
  );
}

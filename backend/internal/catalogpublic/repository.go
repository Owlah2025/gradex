...
func (r *Repository) List(ctx context.Context, arabic bool, page, pageSize int) (ListResult, error) {
	query := r.projectionQuery(``, `ORDER BY c.id LIMIT $2 OFFSET $3`)
...
func (r *Repository) Detail(ctx context.Context, identifier string, arabic bool) (*DetailCourse, error) {
	rows, err := r.pool.Query(ctx, r.projectionQuery(`AND (c.id::text = $2 OR c.slug = $2)`, ``), arabic, identifier)
	if err != nil {
...
func (r *Repository) projectionQuery(identifier, suffix string) string {
	return `SELECT c.id::text, c.slug,
		...
		WHERE ` + r.visibility("c", "cr") + ` ` + identifier + ` ` + suffix
}

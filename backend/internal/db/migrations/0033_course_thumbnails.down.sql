-- Refuse destructive rollback while thumbnail history exists.
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM media_assets WHERE kind='THUMBNAIL') THEN
        RAISE EXCEPTION 'thumbnail assets exist; retain schema 33 and use an application build compatible with that schema';
    END IF;
END $$;
DROP TRIGGER revision_thumbnail_guard ON course_revisions;
DROP FUNCTION revision_thumbnail_guard();
ALTER TABLE course_revisions DROP COLUMN thumbnail_asset_version_id;
DROP INDEX media_thumbnail_single_version;
DROP TABLE media_thumbnail_cleanup;
DROP TABLE media_thumbnail_variants;
DROP FUNCTION thumbnail_variants_immutable();
...
ALTER TABLE media_assets ADD CONSTRAINT media_assets_preview_origin_revision_kind_check CHECK (
    (preview_origin_revision_id IS NULL OR kind = 'PREVIEW') AND (kind <> 'PREVIEW' OR lesson_id IS NULL)
);

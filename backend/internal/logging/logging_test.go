...
func TestRequestCompletedEmitsAgreedFields(t *testing.T) {
	...
	New(&buf, "gradex-api", "production", slog.LevelInfo).RequestCompleted(RequestEvent{
		...
		PasswordVerificationQueued:  true,
		PasswordVerificationOutcome: "ADMITTED",
		LoginCandidateMicros:        123,
		PasswordQueueMicros:         456,
		PasswordVerifyMicros:        789,
		SessionPrepareMicros:        12,
		SessionWriteMicros:          345,
	})
	...
	for field, want := range map[string]any{
		...
		"password_verification_queued":   true,
		"password_verification_outcome":  "ADMITTED",
		"login_candidate_us":             float64(123),
		"password_verification_queue_us": float64(456),
		"password_verification_us":       float64(789),
		"session_prepare_us":             float64(12),
		"session_write_us":               float64(345),
	} {
		if rec[field] != want {

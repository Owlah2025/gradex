...
type CourseRevision struct {
	ThumbnailAssetVersionID *string       `json:"thumbnail_asset_version_id,omitempty"`
	ID                      string        `json:"id"`
	...
	// The selected preview's current media state, projected so a reloading
	// authoring client can tell a still-processing preview from a ready one
	// without holding a poll open. Nil when no preview is selected.
	PreviewAssetState   *string           `json:"preview_asset_state,omitempty"`

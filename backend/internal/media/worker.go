...
func NewWorker(options WorkerOptions) (*Worker, error) {
	...
	return &Worker{db: options.DB, scanner: options.Scanner, process: options.Process, outbox: options.Outbox}, nil
}
...
func (w *Worker) handleScanTask(ctx context.Context, task *asynq.Task) error {
	...
	return w.Scan(ctx, work.AssetVersionID)
}
...
func (w *Worker) handleTranscodeTask(ctx context.Context, task *asynq.Task) error {
	...
	return w.Transcode(ctx, work.AssetVersionID, work.OperationID)
}
...
func (w *Worker) Scan(ctx context.Context, assetVersionID string) error {
	version, attempt, applied, err := w.beginScan(ctx, assetVersionID)
	if err != nil || !applied {
		return err
	}
	...
	if applyErr != nil {
		...
		return w.recordScanFailure(ctx, version, attempt, ScanError, observation.ScannerIdentity, applyErr.Error())
	}
	if err := w.finishScan(ctx, version, attempt, observation, next); err != nil {
		return err
	}
	if scanErr != nil {
		return scanErr
...
func (w *Worker) beginScan(ctx context.Context, assetVersionID string) (versionRecord, int, bool, error) {
...
func (w *Worker) finishScan(ctx context.Context, version versionRecord, attempt int, observation ScanObservation, next AssetVersionState) error {
	tx, err := w.db.Begin(ctx)
	if err != nil {
		return fmt.Errorf("beginning scan result transaction: %w", err)
	}
	defer tx.Rollback(ctx)
	evidence, err := recordScanEvidence(ctx, tx, version, attempt, observation, next)
	if err != nil {
		return err
	}
...
func recordScanEvidence(ctx context.Context, tx pgx.Tx, version versionRecord, attempt int, observation ScanObservation, next AssetVersionState) (scanEvidence, error) {
	commandTag, err := tx.Exec(ctx, `
		INSERT INTO scan_attempts (
			asset_version_id, attempt_number, storage_object_version,
			outcome, scanner_identity, reason
		) VALUES ($1::uuid, $2, $3, $4::media_scan_outcome, $5, NULLIF($6, ''))
		ON CONFLICT (asset_version_id, storage_object_version)
		DO NOTHING
		`, version.ID, attempt, version.Object.StorageObjectVersion, observation.Outcome, observation.ScannerIdentity, observation.Reason)
	if err != nil {
		return scanEvidence{}, fmt.Errorf("recording scan attempt: %w", err)
	}
	if commandTag.RowsAffected() == 0 {
		return loadDuplicateScanEvidence(ctx, tx, version, observation)
	}
	var attemptID string
	if err := tx.QueryRow(ctx, `
		SELECT id::text FROM scan_attempts
		WHERE asset_version_id = $1::uuid AND storage_object_version = $2
	`, version.ID, version.Object.StorageObjectVersion).Scan(&attemptID); err != nil {
		return scanEvidence{}, fmt.Errorf("loading recorded scan evidence: %w", err)
	}
	return scanEvidence{attemptID: attemptID, observation: observation, next: next}, nil
}

func loadDuplicateScanEvidence(ctx context.Context, tx pgx.Tx, version versionRecord, observation ScanObservation) (scanEvidence, error) {
	var evidence scanEvidence
	var persistedOutcome ScanOutcome
	if err := tx.QueryRow(ctx, `
		SELECT id::text, outcome, scanner_identity, COALESCE(reason, '')
		FROM scan_attempts
		WHERE asset_version_id = $1::uuid AND storage_object_version = $2
	`, version.ID, version.Object.StorageObjectVersion).Scan(&evidence.attemptID, &persistedOutcome, &evidence.observation.ScannerIdentity, &evidence.observation.Reason); err != nil {
		return scanEvidence{}, fmt.Errorf("loading duplicate scan evidence: %w", err)
	}
	if persistedOutcome != observation.Outcome {
		return scanEvidence{}, fmt.Errorf("%w: exact object already has a different scan outcome", ErrConflict)
	}
...
func (w *Worker) applyScanState(ctx context.Context, tx pgx.Tx, version versionRecord, evidence scanEvidence) error {
	if evidence.next == StateScanPassed {
		...
		`, evidence.attemptID, version.ID); err != nil {
			return fmt.Errorf("recording successful scan: %w", err)
		}
		...
		if _, err := tx.Exec(ctx, `
			...
			WHERE id = $2::uuid AND state = 'SCANNING'
		`, evidence.next, version.ID); err != nil {
...
func (w *Worker) recordScanFailure(ctx context.Context, version versionRecord, attempt int, outcome ScanOutcome, scannerIdentity, reason string) error {
	return w.finishScan(ctx, version, attempt, ScanObservation{
...
func (w *Worker) Transcode(ctx context.Context, assetVersionID, operationID string) error {
	...
	result, processErr := w.process.Transcode(ctx, version.Object)
	if processErr != nil {

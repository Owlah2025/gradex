...
1. **Names reveal intent.** Avoid unexplained `data`, `data2`, `result`, `result_final`, `item`, `temp`, `value`, `obj`, `info`, `helper`, `manager`, `utils`, or `handle_*`/`process_*`/`do_*` — qualify them (`userResult`), or judge whether the surrounding scope already makes the bare name clear (e.g. a `result` returned two lines later in a five-line function needs no qualifier). A name must answer *why it exists and what it does*. (Clean Code Ch. 2)
...
10. **Prefer defining abstractions in the consumer's package, where the language and existing architecture support it.** This is a project- and language-specific heuristic (a Go idiom for DIP), not a universal rule — follow the repo's established architecture and public API conventions, and never relocate a contract if doing so creates a package cycle or conflicts with existing design. (DIP)
...
20. **Enumerate boundary cases before writing them.** For any range, off-by-one, null/empty/one/many, even/odd, or unicode/byte boundary, list the cases first — as a design checklist or test-case list, never as a code comment. Where a case is meaningfully testable, write it as an executable test, not just a note. Cover every listed case in the implementation before moving on.
21. **Remove code proven unreachable.** A missing local call site is not proof of dead code — confirm unreachability with symbol-aware analysis, not a plain grep, and preserve exported APIs, CLI entry points, plugin/registration hooks, reflection targets, generated-code interfaces, and other external consumer entry points before deleting anything.

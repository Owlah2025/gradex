...
unapproved legal, accounting, payment, or infrastructure position as settled.
...
| `[TAP_CONTACT_NAME]` | Tap account manager or support contact |
| `[OPERATIONS_CONTACT_NAME]` | Operations/support/infrastructure owner |
| `[SENDER_EMAIL]` | Sender's business email |
| `[SENDER_PHONE]` | Sender's business phone, if appropriate |

## Tracking

Do not change a message to `SENT` until it has actually left the sender's account.
...
## Message 4 — Operations and Infrastructure

**To:** `[OPERATIONS_CONTACT_NAME]`

**Subject:** Gradex launch operations and infrastructure inputs

Hello `[OPERATIONS_CONTACT_NAME]`,

Gradex is preparing for a readiness-gated public launch on 15 August 2026. Please help confirm and
execute the following operational inputs:

1. Name the support/community owner and document the support route, moderation rules, escalation
   path, response expectation, and active Discord/Telegram links.
2. Shortlist and validate a malware-scanning service for public previews, Resources, and Lab
   Materials, including supported file types/sizes, quarantine, fail-closed behavior, alerts, and
   test evidence.
3. Select and validate transactional email delivery, including the data-processing boundary,
   sender domain, SPF/DKIM/DMARC, templates and links, bounce/suppression handling, rate limits,
   monitoring, and deliverability evidence.
4. Supply the hosting budget, expected launch load/storage/egress, availability target, RPO/RTO,
   secrets approach, monitoring/alerting needs, incident ownership, backup/restore evidence, and
   representative load-test plan.

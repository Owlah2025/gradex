...
- **BR-009** — Instructors and additional Admins are created only through an existing Admin's expiring invitation. An invitation address already attached to any Account is rejected; MVP never silently changes a role, merges identities, or gives one Account multiple roles. The one bootstrap Admin is created out-of-band during secure deployment, has no credential stored in the repository, and must change the initial password. *(D-014; PRD §11 Authentication)*
...
- **BR-025** — Entitlement lasts 150 days (~5 months) from purchase/grant, calculated in Kuwait local time (UTC+3). It remains valid through the end of day 150 and is denied from day 151. MVP has no renewal flow; a lapsed Student repurchases normally, while Enrollment/progress history remains. *(D-009)*
...
- **BR-122** — Required in-app + email events are purchase receipt, refund status, password/security, Account invitation, Course approval/changes requested, and office-hours cancellation/material rescheduling. New office-hours sessions target currently entitled Students, while new Instructor Course/revision submissions target Admin operations; both are in-app and may also use email when operationally appropriate. Video-processing completion targets the Instructor. *(D-010; PRD §5 Notifications)*

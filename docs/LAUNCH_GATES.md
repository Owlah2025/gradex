...
| LG-014 | Select and validate upload malware scanning | Founder + engineering | Deferred for the bounded trusted-Instructor launch profile under D-088. Malware scanning must be reconsidered before the D-088 trust boundary is broadened | Selected scanner/service, fail-closed quarantine workflow, supported file limits, alerting, and validation evidence when the gate reopens | Public previews and any upload outside the D-088 trusted-Instructor MP4/PDF/DOCX Lesson-media profile | **DEFERRED** |
...
| LG-019 | Approve production operating and recovery envelope | Founder + engineering | Founder approved the launch validation envelope (5,000 accounts, 500 active sessions, 500 Students beginning within one minute, 250 API requests/second for 60 seconds, 250 playback starts/minute, 20 direct uploads, and no more than two transcodes per worker) plus 99.5% monthly availability. Hourly backup and five-minute monitoring timers are implemented but not installed or production-proven. PostgreSQL RPO/RTO remain unapproved | System design/deployment record covering expected launch load/storage/egress, budget, availability, RPO/RTO, managed secrets, monitoring/alerting, incident runbooks, restore test, security review, and representative load test | Production architecture sign-off and release | OPEN |
...
| `LG-014` | Malware scanning is deferred only for the bounded D-088 trusted-Instructor Lesson-media profile. Those MP4/PDF/DOCX uploads may progress only after exact-version validation and without fabricated scan evidence; video still requires trusted processing before `READY`. Public previews and uploads outside that profile remain fail-closed behind the provider-neutral scanner boundary ([API design §7.1](superpowers/specs/2026-07-27-api-security-integration-design.md#71-malware-scanning-adapter)). |
...
| `LG-019` | The Founder-approved launch validation envelope and 99.5% availability target are recorded, but they have not passed representative load validation. Five-minute monitoring and hourly backup schedules are implemented as systemd units. Cost, region, instance sizing, production execution evidence, alert delivery, restore proof, and PostgreSQL RPO/RTO remain open; no WAL/PITR implementation proves the old provisional 15-minute RPO ([platform §§2–3](superpowers/specs/2026-07-25-platform-architecture-design.md#2-architecture-decision)). |

The exam-start identity requirement includes 500 distinct successful browser-equivalent logins in
one minute from the same public IPv4/NAT. Production admission now has a 600/minute shared ceiling,
retains per-identifier/per-browser brute-force limits, and bounds Argon2id locally. This removes the
known static-policy blocker but is not capacity evidence; LG-019 remains `OPEN` pending the external
KVM2 run and resource/session correctness evidence.

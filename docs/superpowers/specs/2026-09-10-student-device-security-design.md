...
6. Device Security owns migration `0036_student_trusted_devices`, the next migration after the
   current release/integration schema 35. The unlanded Bundles/Offers branch has no product
   dependency relationship with this feature and must renumber its competing migration if it lands
   later.
